...
uses
  Classes, SysUtils,CktElement, ucomplex, DSSClass, Spectrum, Arraydef, Meterelement;
...
Begin
    ...
    FOR i := 1 TO Yorder Do begin
      ...
      DoubleCurrents^[2*NodeRef^[i]]:=DoubleCurrents^[2*NodeRef^[i]]+InjCurrent^[i].re;
      DoubleCurrents^[2*NodeRef^[i]+1]:=DoubleCurrents^[2*NodeRef^[i]+1]+InjCurrent^[i].im;
    end;
End;
