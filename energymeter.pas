   ...
   TEnergyMeterObj = class(TMeterElement)
    ...
    // Not used   Procedure RemoveFeederObj;
      ...
      Public
        ...
        SequenceList   :TNPointerList;  // Pointers to branches in sequence from meter to ends
        LoadList       :TNPointerList;  // Pointers to Loads in the Meter zone to aid reliability calcs
...
USES  ParserDel, DSSClassDefs, DSSGlobals, Bus, MathUtil,  UCMatrix,
      Utilities, PCElement,  StackDef, Circuit, Line, LineUnits,  Math,ReduceAlgs;
...
Begin
  ...
  If Assigned(SequenceList) Then  SequenceList.Free;
  SequenceList := PointerList.TNPointerList.Create(1024); //make it a big initial allocation
  If Assigned(LoadList) Then  LoadList.Free;
  LoadList := PointerList.TNPointerList.Create(1024); //make it a big initial allocation
