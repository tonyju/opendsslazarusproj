...
uses
  Classes, SysUtils,uCOMPLEX, Arraydef,
    Command,
    Monitor,
    DSSClass,
    DSSObject,
...
TYPE
   ...
   //TDoublearray= Array[0..2000] of double; //should use pdouble array
...
//       CommandList:TCommandlist;
   ...
   TSolutionObj = class(TDSSObject)
     private
       ...
       Function SolveRealSystem(V:pDoubleArray0):Integer;
...
//       PROCEDURE GetMachineInjCurrents;
       ...
       PROCEDURE ZeroDoubleInjCurr;
       PROCEDURE initDoubleNodeV;
     public
       ...
       hRealYsystem :LongWord;   {Handle for main (system) Y matrix}
       hRealYseries :LongWord;   {Handle for series Y matrix}
       hRealY :LongWord;         {either hYsystem or hYseries}
       ...
       doubleNodeV:pDoubleArray0; // main system voltage array allows NoveV[0],[1]=0,re and im
       doubleCurrents:pDoubleArray0;//main system currents double array
...
USES  SolutionAlgs,
      DSSClassDefs, DSSGlobals,  CktElement,  ControlElem, Fault,
...
Begin
      ...
      Reallocmem(DoubleCurrents, 0);
      Reallocmem(dVdouble, 0);
      Reallocmem(DoubleNodeV, 0);


      //If hYsystem <> 0 THEN   DeleteSparseSet(hYsystem);
      //If hYseries <> 0 THEN   DeleteSparseSet(hYseries);
      //If hRealYsystem <> 0 THEN   DeleteRealSparseSet(hRealYsystem);
      //If hRealYseries <> 0 THEN   DeleteRealSparseSet(hRealYseries);
   ...
   Repeat
    ...
    { Get injcurrents for all PC devices  }
       ...
       // The above call could change the primitive Y matrix, so have to check
        IF SystemYChanged THEN begin
          BuildYMatrix(WHOLEMATRIX, FALSE);  // Does not realloc V, I
            //BuildRealYMatrix(WHOLEMATRIX, FALSE);
        end;
   ...
   Begin
       ...
       REPEAT
        ...
        // Get sum of currents at all nodes for all  devices
           ...
           begin
           BuildYMatrix(WHOLEMATRIX, FALSE);   // Does not realloc V, I
           //BuildRealYMatrix(WHOLEMATRIX, FALSE);   // Does not realloc V, I
           end;
...
Begin
   ...
   htest:=0;
    ...
    begin
           ...
           //ResetRealSparseMatrix(htest,6);
           //BuildRealYMatrix(SERIESONLY, TRUE);
    end;
    ...
    //SolveRealSystem(DoubleNodeV);  // also sets voltages in radial part of the circuit if radial solution

    { Reset the main system Y as the solution matrix}
    IF   (hYsystem > 0) and Not SolutionAbort THEN
      hY := hYsystem;

    IF   (hRealYsystem > 0) and Not SolutionAbort THEN
      hRealY := hRealYsystem;
End;
...
Begin
   ...
   begin
     ...
     //BuildRealYMatrix(WHOLEMATRIX, TRUE);   // Side Effect: Allocates V
   end;
...
PROCEDURE TSolutionObj.ZeroDoubleInjCurr;
VAR
    I:Integer;
Begin
    FOR i := 0 to ActiveCircuit.NumNodes Do begin
      DoubleCurrents^[i] := 0.0;
      DoubleCurrents^[i+1] := 0.0;
      end;
End;

PROCEDURE TSolutionObj.InitDoubleNodeV;
VAR
    I:Integer;
Begin
    FOR i := 0 to ActiveCircuit.NumNodes Do begin
      DoubleNodeV^[i+1]:= NodeV^[i].re;
      DoubleNodeV^[i+1] := NodeV^[i].im;
      end;
End;
...
END;
FUNCTION TSolutionObj.SolveRealSystem(V:pDoubleArray0): Integer;
...
BEGIN
    ...
    {Note: NodeV[0] = 0 + j0 always.  Therefore, pass the address of the element 1 of the array.
    Note: NodeV[0] = 0 + j0 always.  Therefore, pass the address of the element 0,1 of the doublearray.
 }
  Try
    // new function to log KLUSolve.DLL function calls
    // SetLogFile ('KLU_Log.txt', 1);
    writeln('solverealsystem retcode!');
    //0 1 is for ground
    //RetCode :=  SolveRealSparseSet(hRealY, @V^[2], @DoubleCurrents^[2]);  // Solve for present InjCurr
    // new information functions
    // GetFlops (hY, @dRes);
    // GetRGrowth (hY, @dRes);
    //GetRCond (hY, @dRes);
...
BEGIN
   ...
   SolveRealSystem(DoubleNodeV); // Solve with Zero injection current
END;
